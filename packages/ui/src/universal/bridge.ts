...
export interface UpdateStateView { status: string }
export interface OllamaDiagnosisView { state: string; detail: string; checkedAt: string; serverVersion?: string; baseUrl: string }
export interface NarratorStatusView { language: string; speaking: boolean; queueDepth: number; english: unknown; cantonese: unknown }
export interface HistoryAvailabilityView { available: boolean; reason?: string; gitVersion?: string }
export interface RuntimeStatusView { checkedAt: string; secureVaultAvailable: boolean; updater: UpdateStateView; ollama: OllamaDiagnosisView; narrator: NarratorStatusView; history: HistoryAvailabilityView }
...
export interface RevisionView { hash: string; date: string; action?: string; message: string; labels: string[] }
export interface SelectedSourceFileView { handle: string; name: string; size: number }
export interface ConverterEntryView { id: string; category: string; sourceFormat: { id: string; label: string }; targetFormat: { id: string; label: string }; bundled: boolean; unavailableReason?: string; metadataBehavior: string; lossiness: "lossless" | "lossy"; lossDisclosure: readonly { aspect: string; detail: string }[]; userFacingName: string }
...
export interface UniversalHostBridge {
  ...
  settings?: { load?: () => Promise<unknown>; set?: (key: "displayName", value: string) => Promise<unknown>; setMany?: (values: Record<string, unknown>) => Promise<unknown>; resetToDefault?: (key: string) => Promise<unknown>; resetAllToDefaults?: () => Promise<unknown> };
